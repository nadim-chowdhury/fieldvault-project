...
export default function AssetDetailPage({
  ...
  const handleCheckout = (e: React.FormEvent) => {
    ...
    checkoutMut.mutate({
      assetId,
      siteLocation,
      conditionOnCheckout: checkoutCondition,
      notes: checkoutNotes,
    });
  };
  ...
  const handleCheckin = (e: React.FormEvent) => {
    ...
    checkinMut.mutate({
      ...
      payload: {
        ...
        photoOnReturnUrl: returnPhotoUrl,
        notes: checkinNotes,
      },
    });
  };
      ...
      <div className="flex items-center gap-4 mb-8 pb-6 border-b border-slate-100">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3">
            ...
            <span
              className={`text-xs px-2.5 py-1 rounded-full font-medium capitalize ${statusColors[assetData.status] || "bg-slate-100 text-slate-500"}`}
            >
              ...
              <div>
                ...
                <p className="font-medium text-slate-900">
                  ...
                  {assetData.purchaseValue
                    ? Number(assetData.purchaseValue).toLocaleString()
                    : "-"}
                </p>
              </div>
              <div>
            ...
            {assignmentsLoading ? (
              ...
              <div className="text-center py-20">
                ...
                <p className="text-sm text-slate-400 mt-1">
                  Check out this asset to build history
                </p>
              </div>
            ) : (
                  ...
                  >
                    ...
                    <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] bg-white p-4 rounded-xl border border-slate-100 shadow-sm hover:border-slate-300 transition-colors">
                      <div className="flex justify-between text-xs text-slate-400 mb-2">
                        <span>
                          {new Date(asg.checkedOutAt).toLocaleDateString()}
                        </span>
                        {asg.checkedInAt ? (
                          <span className="text-emerald-600 font-medium">
                            In: {new Date(asg.checkedInAt).toLocaleDateString()}
                          </span>
                        ) : (
                      ...
                      <div className="text-xs text-slate-600 space-y-1.5 bg-slate-50 p-3 rounded-lg">
                        <p>
                          ...
                          {asg.user?.name || "Unknown User"}
                        </p>
                        <p>
                          <strong>Out Condition:</strong>{" "}
                          {asg.conditionOnCheckout || "-"}
                        </p>
                        {asg.checkedInAt && (
                          <>
                            <p>
                              <strong>In Condition:</strong>{" "}
                              {asg.conditionOnReturn || "-"}
                            </p>
                            {asg.photoOnReturn && (
                              <a
                                href={asg.photoOnReturn}
                                target="_blank"
                                rel="noreferrer"
                                className="inline-flex items-center gap-1 mt-2 text-blue-600 hover:text-blue-700 bg-blue-50 px-2 py-1 rounded font-medium transition-colors"
                              >
                                <Camera className="w-3.5 h-3.5" /> View Return
                                Photo
                              </a>
                            )}
                          </>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-slideUp">
            <div className="flex items-center justify-between p-5 border-b border-slate-100">
              <h3 className="font-semibold text-slate-900 flex items-center gap-2">
                <ArrowUpRight className="w-5 h-5 text-blue-600" /> Check-out
                Asset
              </h3>
              <button
            ...
            <form onSubmit={handleCheckout} className="p-6 space-y-5">
              <div>
                ...
                <input
                  ...
                  type="text"
                  className="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm placeholder:text-slate-400"
                  placeholder="e.g. Block A, Site 42"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">
                ...
                <input
                  ...
                  type="text"
                  className="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm placeholder:text-slate-400"
                  placeholder="e.g. Good, fully charged"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">
              ...
              <button
                ...
                type="submit"
                className="w-full py-2.5 bg-blue-600 text-white font-medium rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors shadow-sm"
              >
      ...
      {isCheckinModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4 animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-slideUp">
            <div className="flex items-center justify-between p-5 border-b border-slate-100">
              <h3 className="font-semibold text-slate-900 flex items-center gap-2">
                <ArrowDownLeft className="w-5 h-5 text-emerald-600" /> Check-in
                Asset
              </h3>
              <button
            ...
            <form onSubmit={handleCheckin} className="p-6 space-y-5">
              <div>
                ...
                <input
                  ...
                  type="text"
                  className="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none text-sm placeholder:text-slate-400"
                  placeholder="e.g. Minor scratches, battery low"
                />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">
                ...
                <p className="text-xs text-slate-500 mt-1.5 flex items-center gap-1">
                  <Camera className="w-3 h-3" /> Provide a link to the return
                  photo (MVP)
                </p>
              </div>
              <div>
              ...
              <button
                ...
                type="submit"
                className="w-full py-2.5 bg-emerald-600 text-white font-medium rounded-lg hover:bg-emerald-700 disabled:opacity-50 transition-colors shadow-sm"
              >
